/**
  ...
  *-----------------------------------------------------------------------------
  *        SYSCLK(Hz)                             | 72000000 // 37.5 MHz
  *-----------------------------------------------------------------------------
  *        HCLK(Hz)                               | 72000000 // 37.5 MHz
  *-----------------------------------------------------------------------------
  *        AHB Prescaler                          | 1
  *-----------------------------------------------------------------------------
  *        APB2 Prescaler                         | 1
  ...
  *        APB1 Prescaler                         | 2
  *-----------------------------------------------------------------------------
  *        HSE Frequency(Hz)                      | 8000000 // 10 MHz
  *----------------------------------------------------------------------------
  *        PLLMUL                                 | 9 // 15
  *-----------------------------------------------------------------------------
  *        PREDIV                                 | 1 // 4
...
/******************************************************************************/
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    //RCC->CFGR |= (uint32_t)(RCC_CFGR_PLLSRC_PREDIV1 | RCC_CFGR_PLLXTPRE_PREDIV1 | RCC_CFGR_PLLMULL9); // for 72 MHz and 8 MHz quartz
    RCC->CFGR |= (uint32_t)(RCC_CFGR_PLLSRC_PREDIV1 | RCC_CFGR_PLLXTPRE_PREDIV1 | RCC_CFGR_PLLMULL6);    
    /*  Set HSE preddiv = 4  */
    RCC->CFGR2 &= (uint32_t)0xFFFFFFF0; // Reset PREDIV1 configuration
    RCC->CFGR2 |= (uint32_t)/*RCC_CFGR2_PREDIV1_DIV2*/RCC_CFGR2_PREDIV1_DIV1; 
    
    /* Set USART1 clock source */
    RCC->CFGR3 &= (uint32_t)RCC_CFGR3_USART1SW; // Reset USART Clock setting
    RCC->CFGR3 |= (uint32_t)RCC_CFGR3_USART1SW_0; // Set SYSCLK USART clocks
